...
- `.claude/commands/migration.md` — **migration CSDL có kỷ luật**. **TRIGGER:** khi *đổi schema / thêm bảng-cột-index* (hoặc gõ `/migration`) → migration **có phiên bản + rollback được**, tự dò công cụ thật (Supabase/Prisma/Drizzle/SQL…); thay đổi **phá vỡ** → dừng & hỏi (§9), đề xuất phương án expand→migrate→contract.
...
- `.claude/commands/tien-do.md` — **theo dõi tiến độ** (cập nhật `PROGRESS.md` + kiểm cổng giai đoạn). **TRIGGER:** đầu mỗi phiên / sau mỗi mốc / khi *hỏi đang ở đâu, làm gì tiếp* (hoặc gõ `/tien-do`) → cập nhật `PROGRESS.md` (giai đoạn, đã xong/đang làm/tiếp theo, quyết định, nợ kỹ thuật) và đối chiếu cổng trước khi chuyển giai đoạn (CLAUDE.md §2).
